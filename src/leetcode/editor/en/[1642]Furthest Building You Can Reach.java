...
// 
// Related Topics Array Greedy Heap (Priority Queue) 👍 1507 👎 44
...
class Solution {
    public int furthestBuilding(int[] heights, int bricks, int ladders) {
        ...
        int totalBricks = 0, saveBricks = 0;
        int i;
        for (i = 0; i < N - 1; i++) {
            ...
            totalBricks += diff;
            if (ladders > 0 && (pq.size() < ladders || pq.peek() < diff)) {
                pq.offer(diff);
                saveBricks += diff;
                if (pq.size() > ladders) {
                    saveBricks -= pq.poll();
            ...
            if (totalBricks - saveBricks > bricks) break;
        }
        return i;
    }
}
