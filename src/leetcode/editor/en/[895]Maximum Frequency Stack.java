...
//calling pop. 
// 
// Related Topics Hash Table Stack Design Ordered Set 👍 2391 👎 40
...
class FreqStack {
    ...
    TreeMap<Integer, Stack<Integer>> freq2vs = new TreeMap<>();
    Map<Integer, Integer> v2freq = new HashMap<>();
    ...
    public void push(int val) {
        int f = v2freq.getOrDefault(val, 0);
        v2freq.put(val, ++f);

        if (freq2vs.containsKey(f)) {
            freq2vs.get(f).push(val);
        } else {
    ...
    public int pop() {
        int key = freq2vs.lastKey();
        Stack<Integer> stack = freq2vs.get(key);
        int val = stack.pop();
        v2freq.put(val, v2freq.get(val) - 1);
        if (stack.isEmpty()) freq2vs.remove(key);
        return val;
    }
}
