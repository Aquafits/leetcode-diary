package leetcode.editor.en;//Implement the RandomizedSet class:
//
// 
...
//called. 
// 
// Related Topics Array Hash Table Math Design Randomized 👍 4891 👎 265
...
class RandomizedSet{
    private Map<Integer, Integer> v2id = new HashMap<>();
    ...
    public boolean insert(int val) {
        if(v2id.get(val)!=null){
            return false;
        }
        list.add(val);
        v2id.put(val, list.size()-1);
        return true;
    }

    /** Removes a value from the set. Returns true if the set contained the specified element. */
    public boolean remove(int val) {
        Integer rmId = v2id.get(val);
        if(rmId == null){
            return false;
        }
