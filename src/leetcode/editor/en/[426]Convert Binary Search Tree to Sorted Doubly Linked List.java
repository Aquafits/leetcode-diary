...
//Binary Tree Doubly-Linked List 👍 2007 👎 162
...
class Node2List {
    ...
    Node2List(Node node) {
        if (node.left == null) {
            ...
            Node2List leftList = new Node2List(node.left);
            front = leftList.front;
            node.left = leftList.rear;
            leftList.rear.right = node;
        }

        if (node.right == null) {
            rear = node;
        } else {
            Node2List rightList = new Node2List(node.right);
            rear = rightList.rear;
            node.right = rightList.front;
            rightList.front.left = node;
        }
    }
}
...
class Solution {
    public Node treeToDoublyList(Node root) {
        ...
        Node2List lst = new Node2List(root);
        lst.front.left = lst.rear;
        lst.rear.right = lst.front;
        return lst.front;
    }
}
