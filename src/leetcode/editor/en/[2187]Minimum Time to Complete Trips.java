...
class Solution {
    public long minimumTime(int[] time, int N) {
        //  N + 1 > sum(floor(T/t)) >= N
        //  N + 1 + |time| > sum(T/t) >= N
        //  (N + 1 + |time|) / sum(1/t) > T >= N / sum(1/t)
        double dividend = 0;
        for (int t : time) dividend += 1.0 / t;

        long l = (long) (N / dividend) - 1, r = (long) ((N + 1 + time.length) / dividend) + 1;
        while (l < r) {
            long mid = (l + r) >> 1;
            if (countTrips(time, mid) >= N) {
                r = mid;
            } else {
                l = mid + 1;
            }
    ...
    private int countTrips(int[] time, long T) {
        int cnt = 0;
        for (long t : time) {
            cnt += T / t;
        }
