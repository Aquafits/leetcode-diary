...
// 
// Related Topics Array Two Pointers 👍 241 👎 43
...
class Solution {
    public List<List<Integer>> findRLEArray(int[][] encoded1, int[][] encoded2) {
        ...
        while (p1 < encoded1.length && p2 < encoded2.length) {
            int lenOverlap = Math.min(encoded1[p1][1], encoded2[p2][1]);
            int product = encoded1[p1][0] * encoded2[p2][0];

            // calc [l ,r];
            if (preEncode != null && preEncode[0] == product) {
                res.set(res.size() - 1, Arrays.asList(product, preEncode[1] + lenOverlap));
                preEncode[1] = preEncode[1] + lenOverlap;
            } else {
                res.add(Arrays.asList(product, lenOverlap));
                preEncode = new int[]{product, lenOverlap};
            }

            encoded1[p1][1] -= lenOverlap;
            encoded2[p2][1] -= lenOverlap;
